...
def train(
    dataloaders, model, criterion, optimizer, scheduler, device, optim_model_wts_dir, n_epochs=30
):
    """
    ...
    for epoch in range(n_epochs):
        ...
        print(
            "train loss: {:.6f}, val loss: {:.6f}, accuracy: {:.2f}".format(
                train_loss, val_loss, 100 * val_accuracy
            )
        )
        print("-" * 60)
        print()
